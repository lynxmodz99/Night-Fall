
```md
```html
<p align="center">
  <a href="https://ibb.co/zTDB9315">
    <img src="https://i.ibb.co/pB5MVHgP/Screenshot-2026-06-10-182114.png" alt="NIGHT FALL Banner" width="100%">
  </a>
</p>

<h1 align="center">🌙 NIGHT FALL</h1>

<p align="center">
  <b>A Powerful Python-Based Discord Utility Suite</b><br>
  Fast • Powerful • Minimal
</p>
```



## 📥 Installation

### 1. Clone the Repository OR download it

### 2. Install Python Dependencies

```bash
pip install -r requirements.txt
```

### 3. Install FFmpeg (Required)

For voice-related features, FFmpeg is required.

**Official FFmpeg Website:**
- https://ffmpeg.org/download.html

**Recommended Windows Build (Officially Referenced):**
- https://www.gyan.dev/ffmpeg/builds/

### 📌 FFmpeg Setup

1. Open the Gyan FFmpeg Builds page.
2. Download the latest **`ffmpeg-release-essentials.zip`** (Windows `.zip` build).
3. Extract the archive.
4. Open the extracted folder and navigate to the `bin` folder.
5. Copy **`ffmpeg.exe`**, **`ffplay.exe`**, and **`ffprobe.exe`**.
6. Paste these files into the **root folder** of this project (the same folder where `main.py` is located).

### 4. Run the Project

```bash
python main.py
```

---

## 📖 Requirements

- Python **3.11+**
- Windows 10/11
- Stable Internet Connection
- FFmpeg installed (see above)

---

##  Support

If you enjoy this project, please consider giving it a **⭐ Star** on GitHub.  
It helps support future updates and development.
Join support server - https://dsc.gg/lynx-modz
---

<p align="center">
  Made with 🖤 by <b>LYNX</b>
</p>
````
